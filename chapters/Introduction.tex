...
First of all we want to draw some conclusions about the convergence of the Riemann zeta function and its holomorphy on its domain.
...
\end{corollary}
\begin{proof}
	Since $f(n) = n^{-s}$ is a completely multiplicative arithmetic function, $\abs{f(n)} = n^{-\sigma} < 1$ for $\sigma > 1$ and the series converges absolutely the above statement follows directly from the previous theorem.
\end{proof}
